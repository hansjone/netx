const zh = {
  ...
  workbench: {
    ...
    fleetTopoEdges: "边",
    fleetTopoOther: "其他区域",
    fleetTopoUnassigned: "未分区",
    fleetTopoNoRegion: "暂无区域",
    fleetTopoDiscover: "最近发现",
    fleetTopoUpdated: "摘要更新",
    fleetSched: "调度",
    fleetSchedConfig: "配置同步",
    fleetSchedLldp: "LLDP",
    fleetSchedPort: "端口流量",
    fleetSchedNeCollect: "网元采集",
    fleetSchedFabric: "拓扑对账",
    fleetSchedOn: "ON",
    fleetSchedOff: "OFF",
    fleetSchedMode: "模式",
    fleetSchedHeartbeat: "心跳",
    fleetSchedFresh: "新鲜",
    fleetSchedStale: "过期",
  ...
  bizState: {
    ...
    createModeImport: "手动导入",
    createImportHint:
      "无网元也可建任务：上传会话日志或 zip，自动切分 show/display 并解析为首个批次。之后可在任务上多次「导入日志」追加批次（等同于多次采集）。",
    createImportSubmit: "创建并导入",
    created: "已创建任务",
    createFailed: "创建失败",
    empty: "暂无业务监控任务。",
    pickNe: "选择网元…",
    neKeywordPh: "名称 / IP / ID",
    allSource: "全部来源",
    selectedNe: "已选",
    neEmpty: "无匹配网元。",
    colSource: "来源",
    ...
    importLog: "导入日志",
    importLogHint:
      "向当前任务追加一批次（等同于一次采集）：支持整段会话或 zip，丢掉 show/display 前杂讯，按 hostname#show 切分；整份无 show 则失败（无需 SSH）。",
    importVendor: "厂商",
    importVendorGeneric: "通用（show+display）",
    importFile: "日志文件",
    importNeName: "任务名称（可选）",
    importNeNamePh: "例如：PE1-offline",
    importSubmit: "导入为新批次",
    importing: "导入中",
    importStarted: "已开始导入（{{count}} 段命令）",
    importOfflineOnly: "导入任务仅支持离线导入，无 SSH 采集",
    sourceImport: "导入",
    stopCollect: "停止采集",
    ...
    colAlias: "别名",
    setAlias: "设别名",
    aliasPrompt: "批次别名（比对/割接选择时优先显示；留空清除）",
    aliasSaved: "别名已保存",
    aliasEmpty: "未命名",
    workbookStats: "对比项 {{compare}} · 有数据 {{withData}} · 表格 {{sheets}}",
    deleteBatch: "删除",
    bulkDelete: "批量删除",
    ...
    bindTitle: "选择参数关联",
    unbound: "未关联",
    bindHintRequired: "需关联参数后才可采集",
    paramsDblClickHint: "单击查看全部关联明细",
    paramsClickHint: "单击查看全部关联明细",
    paramsDetailTitle: "参数关联明细",
    paramsDetailCount: "共 {{n}} 条",
    discoverLoading: "正在发现…",
    discoverEmpty: "未发现可用选项",
    discoverCacheHit: "已复用（未重新登录）",
    discoverRefresh: "重新从设备拉取",
    selectAllVrfs: "全选",
    deselectAllVrfs: "取消全选",
    batches: "采集批次",
    viewBatch: "查看",
    batchDetail: "详情",
    export: "导出",
    ...
    exportRawLog: "导出为文本",
    exportRawLogOk: "日志已导出",
    noBatches: "尚无批次",
    batchCollectDetail: "采集详情",
    batchWorkbook: "批次工作簿",
    batchId: "批次 ID",
    copyBatchId: "复制批次 ID",
    batchMessageEmpty: "无额外说明或报错",
    batchCmdSummary:
      "命令 {{ok}} 成功 / {{fail}} 失败 / {{skipped}} 跳过 / {{other}} 其他 · 统计 {{total}} 条（批次计数 {{batchCmds}}）",
    sheetCommands: "命令",
    sheetLldp: "LLDP 邻居",
    ...
    rawLogTitle: "原始采集日志",
    rawLogEmpty: "无原始输出",
    rawLogStats: "采集 {{lines}} 行 · 入库 {{rows}} 条",
    rawLogDeclared: "声明 {{declared}}",
    openWorkbook: "打开工作簿",
    filterColumn: "筛选列",
    filterAllCols: "全部列",
  ...
  bizCompare: {
    ...
    field: "字段",
    note: "备注",
    keyRequired: "至少选择一个 Key 字段",
    keyIsIdentity: "Key 用于对齐行，不作为值比对字段",
    metricsRequired: "请至少添加一张监控表",
    noMetricFields: "该 metric 无字段定义",
    rowFilters: "行过滤规则",
    rowFiltersHint:
    ...
    normalizeMac: "MAC",
    normalizeEmpty: "空值归一",
    templateFilterPh: "筛选模板名 / metric",
    jobFilterPh: "筛选任务名 / 模板 / 状态",
    addMetric: "添加监控项",
    pickMetric: "选择监控项…",
    addSheet: "加入模板",
    removeSheet: "移出本表",
    ...
    kindAddedShort: "新增",
    kindRemovedShort: "缺失",
    kindChangedShort: "不一致",
    kindUnchangedShort: "成功",
    kindFail: "失败",
    kindPass: "成功",
    kindSuccess: "成功",
    successCount: "成功",
  ...
  bizMigration: {
    ...
    projects: "任务",
    empty: "暂无割接任务。",
    emptyProjects: "暂无割接任务",
    listFilterPh: "筛选任务名 / 网元 / IP / 状态",
    kpiProjects: "任务数",
    kpiActive: "进行中",
    detail: "详情",
    cancel: "关闭",
  ...
  lldpLinks: {
    ...
    policySaved: "策略已保存",
    enabled: "启用周期调度",
    autoAddUnmatched: "发现时自动入库未匹配网元（占位）",
    autoAddUnmatchedHint: "周期采集与画布单点发现共用；关闭则未匹配邻居只记未匹配、不建占位网元。默认开启。",
    intervalDays: "周期（天）",
    interval: "采集周期",
    intervalUnitDays: "天",
    intervalUnitHours: "小时",
