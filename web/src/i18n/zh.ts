const zh = {
  ...
  bizCompare: {
    ...
    emptyTemplates: "暂无模板。可新建或使用系统默认。",
    templateHint:
      "一个模板可包含多张监控表。Key：对齐前后条目（选中后即为行身份，无需再勾比对）；接口：参与端口映射；比对：勾选具体参数做值检查。若不勾任何比对字段，则只校验条目是否一致（存在性）。",
    keyFields: "Key",
    ifaceFields: "接口",
    compareFields: "比对",
    ignoreFields: "忽略",
    ...
    mapHint: "可选。不填表示忽略端口变化（同设备比对按非接口 Key 对齐）。填写时每行 before_if,after_if",
    mappingSaved: "映射已保存",
    mappingOptionalHint: "未填端口映射：忽略本端端口变化，仅按邻居等非接口 Key 对齐。",
    createJob: "建立比对任务",
    defaultJobName: "割接比对",
    jobName: "名称",
    template: "模板",
