const en = {
  ...
  bizCompare: {
    ...
    emptyTemplates: "No templates yet. Create one or use built-ins.",
    templateHint:
      "A template holds multiple metric sheets. Key: row identity (no separate compare). Interface: port mapping. Compare: value checks after key match. Leave compare empty for presence-only (entry set match).",
    keyFields: "Key",
    ...
    mapHint:
      "Optional. Leave empty to ignore port renames (match on non-interface keys). Or one before_if,after_if pair per line",
    mappingSaved: "Mapping saved",
    mappingOptionalHint: "No port map: ignore local port changes; match on non-interface keys.",
